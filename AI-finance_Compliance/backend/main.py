...
from backend.services.compliance_guard import check_portfolio_compliance, ComplianceReport, FUND_UNDERLYING_HOLDINGS
...
class PortfolioAllocateRequest(BaseModel):
    ...
    holding_period_years: int = Field(default=5, ge=0, description="Intended holding period in years")
        ...
        while loop_count < max_loops:
            ...
            for item in portfolio.allocations:
                if item.fund_code in FUND_UNDERLYING_HOLDINGS:
                    for stock_ticker in FUND_UNDERLYING_HOLDINGS[item.fund_code]:
                        esg_client.get_esg_report(stock_ticker, session_id=sess_id)
                else:
            ...
            if compliance.is_compliant:
                ...
                elif any(word in "".join(compliance.violations) for word in ["Downgraded Stock", "Non-JUMP+ Stock", "SEC Enforcement"]):
                    # Identify and exclude the non-compliant fund that holds the non-compliant stock
                    non_compliant_found = False
                    for item in portfolio.allocations:
                        if any(item.fund_code in v for v in compliance.violations):
                            trace.append(f"Self-Healing Action: Excluding non-compliant fund {item.fund_code} from allocation.")
                            current_instructions += f" avoid {item.fund_code}"
                            non_compliant_found = True
                    if not non_compliant_found:
                        trace.append("Self-Healing Action: Standard compliance correction.")
                        break
                else:
